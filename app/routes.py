...
def register():
    ...
    if user_check:
        flash('That username and/or email address is already registered')
        return redirected(url_for('register'))
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is not None and user.check_password(password):
            login_user(user)
            flash(f'Succesfully logged in with {email}', 'success')
        else:
...
def create_coffee():
    ...
    if form.validate_on_submit():
        ...
        db.session.comit()
        flash(f'You have created the {new_coffee} coffee!', 'success')
        return redirect(url_for('index'))
...
def delete_coffee(coffee_id):
    ...
    if coffee is None:
        ...
        flash(f'You do not have permission to delete this coffee', 'danger')
        return redirect(url_for('index'))
